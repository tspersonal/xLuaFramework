...
//
//	description:	TimerCount is a script extension for delaying (scheduling) methods
...
public class TimerCount : MonoBehaviour
{
    ...
    private static List<Event> m_ActiveDotDestroryOnChangeMap = new List<Event>();
    ...
    /// <summary>
    /// returns false if this script was not added via the 'TimerCount.In'
    ...
    /// <summary>
    /// in Awake a check is made to see if the TimerCount component
    ...
    private void Update()
    {
        ...
        m_EventBatch = 0;
        while ((TimerCount.m_Active.Count > 0) && m_EventBatch < MaxEventsPerFrame)
        {
            ...
            if (m_EventIterator < 0)
            {
                ...
                m_EventIterator = TimerCount.m_Active.Count - 1;
                break;
            }

            // prevent index out of bounds
            if (m_EventIterator > TimerCount.m_Active.Count - 1)
                m_EventIterator = TimerCount.m_Active.Count - 1;

            // execute all due events
            if (Time.time >= TimerCount.m_Active[m_EventIterator].DueTime ||    // time is up
                TimerCount.m_Active[m_EventIterator].Id == 0)                   // event has been canceled ('Execute' will kill it)
                TimerCount.m_Active[m_EventIterator].Execute();
            else
            {
                // handle pausing
                if (TimerCount.m_Active[m_EventIterator].Paused)
                    TimerCount.m_Active[m_EventIterator].DueTime += Time.deltaTime;
                else
                    // log lifetime
                    TimerCount.m_Active[m_EventIterator].LifeTime += Time.deltaTime;
            }
        ...
        for (int i = 0; i < m_ActiveDotDestroryOnChangeMap.Count; i++)
        {
            Event time = m_ActiveDotDestroryOnChangeMap[i];
            if (Time.time >= time.DueTime || // time is up
                time.Id == 0) // event has been canceled ('Execute' will kill it)
            {
                time.Execute();
                m_ActiveDotDestroryOnChangeMap.RemoveAt(i);
                i--;
            }
    ...
    public static int InNotDestroy(float delay, Callback callback)
    {
        m_NewEvent = new Event();
        if (m_MainObject == null)
        {
            m_MainObject = new GameObject("Timers");
            m_MainObject.AddComponent<TimerCount>();
        ...
        TimerCount.m_EventCount++;
        m_NewEvent.Id = TimerCount.m_EventCount;
        m_NewEvent.DueTime = Time.time + delay;

        // set up the event with its function, arguments and times
        if (callback != null)
            m_NewEvent.Function = callback;
        m_ActiveDotDestroryOnChangeMap.Add(m_NewEvent);
        return m_NewEvent.Id;
    }

    public static void RemoveInNotDestroy(int nid)
    {
        for (int i = 0; i < m_ActiveDotDestroryOnChangeMap.Count; i++)
        {
            if (m_ActiveDotDestroryOnChangeMap[i].Id == nid)
            {
                m_ActiveDotDestroryOnChangeMap.RemoveAt(i);
                return;
            }
    ...
    private static void Schedule(float time, Callback func, ArgCallback argFunc, object args, Handle timerHandle, int iterations, float interval)
    {
        ...
        {
            Debug.LogError("Error: (TimerCount) Aborted event because function is null.");
            return;
        }
        ...
        TimerCount.m_Active.Add(m_NewEvent);
    ...
    /// </summary>
    private static void Cancel(TimerCount.Handle handle)
    {
    ...
    public static void CancelAll()
    {
        ...
        for (int t = TimerCount.m_Active.Count - 1; t > -1; t--)
        {
            TimerCount.m_Active[t].Id = 0;
        }
    ...
    public static void CancelAll(string methodName)
    {
        ...
        for (int t = TimerCount.m_Active.Count - 1; t > -1; t--)
        {
            if (TimerCount.m_Active[t].MethodName == methodName)
                TimerCount.m_Active[t].Id = 0;
        }
    ...
    {
        TimerCount.m_Active.Clear();
        TimerCount.m_Pool.Clear();
    ...
    /// <summary>
    /// provides the custom TimerCount editor class with debug info
    /// </summary>
    ...
    /// <summary>
    /// provides the custom TimerCount editor class with the name of a
    ...
    /// <summary>
    /// provides the custom TimerCount editor class with the id of a
    ...
    //					an event post initiation, schedule it with a 'TimerCount.Handle'
    //
    /////////////////////////////////////////////////////////////////////////////////
    private class Event
    {
        ...
        public void Execute()
        {
            //string sLog =
            //    "TimerCount => ID(序号): {0}\nIterations(迭代次数): {1}\nInterval(间隔): {2}\nDueTime(响应时间): {3}\nStartTime(开始时间): {4}\nLifeTime(存在时间): {5}";
            //Debug.Log(string.Format(sLog, Id, Iterations, Interval, DueTime, StartTime, LifeTime));
            ...
            {
                Error("Function Exception: " + funException.Message);
                Recycle();
                return;
            }
        ...
        /// <summary>
        /// performs internal recycling of the TimerCount
        /// </summary>
        private void Recycle()
        {
            ...
            if (TimerCount.m_Active.Remove(this))
                m_Pool.Add(this);
        ...
        private void Destroy()
        {
            ...
            TimerCount.m_Active.Remove(this);
            TimerCount.m_Pool.Remove(this);
        ...
        private void Error(string message)
        {
            ...
            string msg = "Error: (TimerCount.Event) " + message;
#if (UNITY_EDITOR && DEBUG)
            msg += MethodInfo;
#endif
            Debug.LogError(msg);
    ...
    public class Handle
    {
        ...
        private TimerCount.Event m_Event = null;    // timer we're pointing at
        ...
        public int Id
        {
            ...
            set
            {
                ...
                for (int t = TimerCount.m_Active.Count - 1; t > -1; t--)
                {
                    if (TimerCount.m_Active[t].Id == m_Id)
                    {
                        m_Event = TimerCount.m_Active[t];
                        break;
                    }
                }
                if (m_Event == null)
                    Debug.LogError("Error: (TimerCount.Handle) Failed to assign event with Id '" + m_Id + "'.");
